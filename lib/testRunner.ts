...
export async function extractPageSummary(page: Page): Promise<string> {
  try {
    const elements = await page.evaluate(() => {
      const selectors = ['button', 'a[href]', 'input', 'select', 'textarea', '[role="button"]', '[data-testid]'];
      ...
      return nodes.slice(0, 40).map((el) => {
        ...
        const safeClasses = classList
          ? classList.split(/\s+/).filter((c) => safeIdentifier.test(c)).slice(0, 2)
          : [];
...
export async function runAssertions(page: Page, assertions: Assertion[]): Promise<{
  passed: boolean;
