...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: error.message || "Failed to prepare recording" }, { status: 500 });
  }
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: error.message || "Failed to check recording status" }, { status: 500 });
  }
}
