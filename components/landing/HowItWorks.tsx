    ...
    <div className={cardClass}>
      ...
      <div className="space-y-1 rounded-lg border border-(--landing-border) bg-(--landing-bg) px-3 py-2.5 font-mono text-xs leading-relaxed">
        ...
        <div className="text-(--landing-ink-muted)">
          &nbsp;&nbsp;model: <span className="text-(--landing-ink)">&quot;gemini-3.1-flash-lite&quot;</span>
        </div>
      </div>
...
const steps = [
  {
    ...
    description: "Sign in with GitHub and pick a repository. Only read access to your code is required.",
  ...
  },
  {
    ...
    description: "Edit any generated test case, then run it and see the pass or fail result instantly.",
          ...
          {steps.map((step, index) => (
            <Reveal key={step.number} delay={index * 0.15}>
              <div className="relative z-10">
                <span className="absolute -left-2 -top-2 z-10 flex h-6 w-6 items-center justify-center rounded-full border border-(--landing-border) bg-(--landing-bg-raised) text-[11px] font-semibold text-(--landing-ink-muted)">
                  {step.number}
                </span>
                {step.vignette}
                <h3 className="mt-5 text-xl font-semibold text-(--landing-ink)">{step.title}</h3>
                <p className="mt-2 text-base leading-relaxed text-justify text-(--landing-ink-muted)">{step.description}</p>
              </div>
            </Reveal>
          ))}
        </div>
