  ...
  return (
    <section className={`relative overflow-hidden ${instrumentSerif.variable}`}>
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(circle, var(--landing-accent-soft), transparent 70%)',
        }}
      />
          ...
          <div>
            <Reveal>
              <span className="relative inline-flex items-center gap-1.5 rounded-full border border-(--landing-accent)/25 bg-(--landing-accent-softer) px-3.5 py-1.5 text-xs font-semibold tracking-wide text-(--landing-ink)">
                <span
                  ...
                  style={{
                    background:
                      'radial-gradient(circle, var(--landing-accent-soft), transparent 70%)',
                  }}
                />
                <span>GITHUB</span>
            ...
            <Reveal delay={0.08}>
              <h1
                ...
                className="mt-5 text-4xl leading-[1.15] font-normal tracking-tight text-(--landing-ink) md:text-5xl lg:text-[3.25rem]"
              >
                Test coverage <em className="text-(--landing-accent) italic">without</em> writing
                tests by hand.
              </h1>
            </Reveal>

            <Reveal delay={0.16}>
              <p className="mt-7 max-w-[58ch] text-xl leading-relaxed text-(--landing-ink-muted)">
                Connect a repo. Gemini drafts the test cases. Browserbase runs them in a real
                browser.
              </p>
            </Reveal>
            ...
            <Reveal delay={0.24}>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Button
                  asChild
                  size="lg"
                  className="gap-2 bg-(--landing-accent) hover:bg-(--landing-accent)/90"
                >
                  <Link href="/workspace">
                ...
                <Button
                  ...
                  variant="outline"
                  size="lg"
                  className="gap-2 border-(--landing-border)"
                >
                  <a href="#how-it-works">
          ...
          <div className="mt-16 grid grid-cols-1 gap-4 sm:grid-cols-3 lg:mt-20">
            {kpis.map((kpi) => (
              ...
              >
                ...
                <p className="mt-1 text-xs leading-relaxed text-(--landing-ink-muted)">
                  {kpi.label}
                </p>
              </div>
            ))}
